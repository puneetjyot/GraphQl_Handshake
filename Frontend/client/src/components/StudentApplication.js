...
import { getAppliedJobs } from '../query/queries';
 ...
 class StudentApplication extends Component {
     state = { 
      ...
      async componentDidMount()
      {
       ...
       const { data } = await this.props.client.query({
         ...
         variables: { student_id: localStorage.getItem("loginId")},
         fetchPolicy: "no-cache",
       });
       this.setState({ jobarr: data.getAppliedJobs });
       this.setState({ perjobarr: data.getAppliedJobs });
       this.setState({jobobj:data.getAppliedJobs[0]})
       console.log(data);
   ...
   console.log(value)
    ...
    console.log( i.company_name.indexOf(value))
     if(i.job_title.toLowerCase().indexOf(value)>=0||i.company_name.toLowerCase().indexOf(value)>=0
     || i.job_title.indexOf(value)>=0||i.company_name.indexOf(value)>=0
     )
     {
       result.push(i)
     }
            ...
            <h2 className='ml-5' style={{fontSize:'20px',fontWeight:'600'}}>Your Applications</h2>
              ...
              {this.state.jobarr?this.state.jobarr.map(i=>(
               ...
               <div key={i._id}>
                ...
                <h3 className='ml-2' style={{fontSize:'16px',fontWeight:'700'}}>{i.job_id.job_title}</h3>

                </div>
                <h3 style={{fontSize:'16px',fontWeight:'400'}}>{i.job_id.company_name}</h3>
                <h3 style={{color:"rgba(0,0,0,.56)",fontWeight:"200px",fontSize:'14px'}}>{i.job_id.job_category} Job</h3>
                 <h3 style={{color:"rgba(0,0,0,.56)",fontWeight:"200px",fontSize:'14px'}}>Applied for this job on {i?i.createdAt:''} </h3>
                </div>
                </div>
))
:''}
 ...
 export default withApollo(StudentApplication);